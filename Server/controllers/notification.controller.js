...
exports.update = async(req, res, next) => {
    try {
        ...
        const notification = await Notification.findByIdAndUpdate(_id, { $set: { title, information, users } }, { new: true });
        if (!notification) {
            return res.status(400).json({
                error: true,
...
exports.deleteNotification = async(req, res, next) => {
    try {
        const _id = req.params.id;
        const notification = await Notification.findByIdAndUpdate(_id, { $set: { deleteAt: new Date() } });
        if (!notification) {
            return res.status(400).json({
                error: true,
                message: "notification is not found"
...
exports.getAll = async(req, res, next) => {
    try {
        const notification = await Notification.find({ deleteAt: undefined });
        return res.status(200).json({
            error: false,
